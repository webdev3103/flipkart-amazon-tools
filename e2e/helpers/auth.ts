...
export async function login(page: Page, credentials = DEMO_CREDENTIALS) {
  ...
  const authInjected = await page.evaluate((email) => {
    ...
    const mockUser = {
      ...
      getIdTokenResult: async () => ({ token: 'mock-id-token', claims: {}, authTime: '', signInProvider: '', issuedAtTime: '', expirationTime: '' }),
      reload: async () => {},
      toJSON: () => ({ uid: 'test-user', email: 'test@example.com', displayName: null }),
      phoneNumber: null,
      photoURL: null,
      providerId: 'firebase',
    };
